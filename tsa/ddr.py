...
class DataDrivenRelationshipIndex:
    def bulk_index(self, data) -> None:
        ...
        for record in data:
            ...
            if iri1 is None or len(iri1) == 0 or iri2 is None or len(iri2) == 0 or relationship_type is None or len(relationship_type) == 0:
                continue
        ...
        if len(cleaned) > 0:
            try:
                ...
                logging.getLogger(__name__).exception("Failed do commit, rolling back DataDrivenRelationshipIndex bulk index")
                db_session.rollback()
    ...
    def index(self, relationship_type: str, iri1: str, iri2: str) -> None:
        ...
        try:
            ...
            logging.getLogger(__name__).exception("Failed do commit, rolling back DataDrivenRelationshipIndex index")
            db_session.rollback()
...
class ConceptIndex:
    ...
    def bulk_insert(self, data):
        if data is None or len(data) == 0:
            return
        clean_data = [{'iri': iri} for iri in data if (iri is not None) and (len(iri) > 0)]
        if len(clean_data) == 0:
            return
        ...
        try:
            ...
            logging.getLogger(__name__).exception("Failed do commit, rolling back ConceptIndex bulk index")
            db_session.rollback()
    ...
    def index(self, iri: str) -> None:
        try:
            ...
            logging.getLogger(__name__).exception("Failed do commit, rolling back ConceptIndex index")
            db_session.rollback()
...
class DataCubeDefinitionIndex:
    def index(self, dsd: List[Dict], iri: str) -> None:
        ...
        try:
            ...
            logging.getLogger(__name__).exception("Failed do commit, rolling back DataCubeDefinitionIndex index")
            db_session.rollback()
