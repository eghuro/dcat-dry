...
class Extractor:
    ...
    def __store_to_db(self) -> None:
        ...
        try:
            ...
            if len(self.__db_distributions) > 0:
                db_session.execute(insert(DatasetDistribution).values(self.__db_distributions))
    ...
    def __process_dataset(self, dataset: str) -> None:
        ...
        if effective_dataset_iri is None:
            return
        for distribution in self.__graph.objects(URIRef(dataset), Extractor.dcat.distribution):
    ...
    def __process_distribution(self, distribution: URIRef, effective_dataset_iri: str) -> None:
        queue = self.__get_queue(distribution)
        for download_url in self.__graph.objects(distribution, Extractor.dcat.downloadURL):
            self.__process_distribution_url(str(download_url), effective_dataset_iri, queue)
    ...
    def __process_service(self, service: URIRef, effective_dataset_iri: str):
        ...
        for endpoint in self.__graph.objects(service, Extractor.dcat.endpointURL):
            ...
            if not check_iri(url) or filter_iri(url):
                return
            self.__db_endpoints.append({
                'endpoint': url,
                'ds': effective_dataset_iri
            })


    def __process_distribution_url(self, url: str, effective_dataset_iri: str, queue: list) -> None:
        if not check_iri(url) or filter_iri(url):
            return
        if url.endswith("/sparql"):
            Extractor.log.info("Guessing %s is a SPARQL endpoint, will use for dereferences from effective DCAT dataset %s", url, effective_dataset_iri)
            self.__db_endpoints.append({
                'endpoint': url,
                'ds': effective_dataset_iri
            })
    ...
    def __get_queue(self, distribution: URIRef) -> list:
        ...
        for format in self.__graph.objects(distribution, Extractor.dcterms["format"]):
            if str(format) in Extractor.format_priority:
                return self.__distributions_priority
        for distribution_format in self.__graph.objects(distribution, Extractor.nkod.mediaType):
